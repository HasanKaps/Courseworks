...
void showPPM(struct PPM * bit);

//function to encode
struct PPM * encode (struct PPM * bit, char * message, unsigned int size, unsigned int code
);

//function to decode
char * decode(struct PPM * bit, unsigned int code
);
...
int getPixel(int PrimeNo, int i, int photoSize);

int change(int num);

//
int getPrimeNo(int num);

int isPrime(int prime);

char* binToStr (char * bin);

void createPPM(struct PPM * ppm, char * FileName);
...
int main(int args, char ** argv) {
    ...
    struct PPM * encodedPPM = encode(ppm, "They say she's in the class A-Team", strlen("They say she's in the class A-Team"), 123456);

    char * message = decode(encodedPPM, 123456);


    printf("\n%s\n", message);

    createPPM(encodedPPM, "modified.ppm");
...
)
{
    ...
    createPPM(bit, "bit.ppm");
    int photoSize = bit->width * bit->height;
    int PrimeNo = getPrimeNo((code
     % photoSize)/10);
